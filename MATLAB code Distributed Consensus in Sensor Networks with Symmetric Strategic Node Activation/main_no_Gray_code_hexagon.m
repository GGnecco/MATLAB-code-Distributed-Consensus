...
%saveas(gcf,'figure1_hexagon.pdf');
print('figure1_hexagon','-dpdf','-bestfit');
...
optimal_configuration=dec2bin(find(value>=max(optimal_value)-tolerance)-1,N_nodes)-48
...
set(gcf, 'Position', get(0, 'Screensize'));
%saveas(gcf,'figure2_hexagon.pdf');
%print('figure2_hexagon','-dpdf','-bestfit');
...
for p_index=1:size(p_train,2) %keep N_nodes+1 values for p, equally spaced (with the idea of fitting a polynomial of degree n_nodes)
    for i=1:N_nodes %here one supposes that one of the N_nodes transmitters decides to be inactive, and is assigned randomly
        ...
        for k=0:2^(N_nodes-1)-1
            ...
            symmetric_Nash_code=dec2bin(k,N_nodes-1)-48;
...
legend(['expected utility of one-node activation' newline 'for zero unit activation cost ($\beta(p)-\alpha(p)$)'],'unit activation cost ($k$)','FontSize',14,'Location','Northwest','interpreter','latex');
...
set(gcf, 'Position', get(0, 'Screensize'));
%saveas(gcf,'figure3_hexagon.pdf');
%print('figure3_hexagon','-dpdf','-bestfit');
...
for p_index=1:size(p_train,2)
    ...
    for k=0:2^N_nodes-1
        ...
        optimal_symmetric_mixed_strategy_code=dec2bin(k,N_nodes)-48
...
set(gcf, 'Position', get(0, 'Screensize'));
%saveas(gcf,'figure4_hexagon.pdf');
%print('figure4_hexagon','-dpdf','-bestfit');
...
set(gcf, 'Position', get(0, 'Screensize'));
%saveas(gcf,'figure5_hexagon.pdf');
%print('figure5_hexagon','-dpdf','-bestfit');
...
set(gcf, 'Position', get(0, 'Screensize'));
%saveas(gcf,'figure6_hexagon.pdf');
%print('figure6_hexagon','-dpdf','-bestfit');
...
set(gcf, 'Position', get(0, 'Screensize'));
%saveas(gcf,'figure7_hexagon.pdf');
%print('figure7_hexagon','-dpdf','-bestfit');

toc

save workspace_hexagon
%load workspace_hexagon