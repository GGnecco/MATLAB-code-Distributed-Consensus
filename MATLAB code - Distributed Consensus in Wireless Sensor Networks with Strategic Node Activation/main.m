...
%camroll(-5)
title('network topology','interpreter','latex')

%saveas(gcf,'figure1.pdf');
saveas(gcf,'figure1var.pdf');
...
for i=0:N_nodes
    ...
    legend_string=strvcat(legend_string,sprintf('$n_{active}$=%.1d',i));
end
...
legend(legend_string,'FontSize',11,'Location','Northwest','interpreter','latex');

xlabel('unit activation cost ($k$)','interpreter','latex','FontSize',11)
%ylabel('minimum penalty 2 (convergence time + total activation cost over the nodes)','interpreter','latex')
ylabel('minimum of $c_{global}$','interpreter','latex','FontSize',11)
...
%legend('marginal expected utility of activation','marginal cost of activation','FontSize',11,'interpreter','latex');
legend(['expected utility of one-node activation' newline 'for zero unit activation cost ($\beta(p)-\alpha(p)$)'],'unit activation cost ($k$)','FontSize',11,'Location','South','interpreter','latex');

xlabel('common probability of activation ($p$)','interpreter','latex','FontSize',11)
ylabel('$\beta-\alpha$, $k$','interpreter','latex','FontSize',11)
...
for p_index=1:size(p_train,2)
    ...
    for k=0:2^N_nodes-1
        ...
        expected_utility(p_index)=expected_utility(p_index)+probability_configuration(k+1)*value_optimal_symmetric_mixed_strategy(k+1);
    end
end
...
legend('expected global cost for zero unit activation cost ($\delta(p)$)','expected total cost of activation ($k n p$)','sum ($\delta(p) + k n p$)','Location','Northeast','FontSize',11,'interpreter','latex');

ylim([0,100]);

xlabel('common probability of activation ($p$)','interpreter','latex','FontSize',11)
ylabel('$\delta$, $k n p$, $\delta+k n p$','interpreter','latex','FontSize',11)
...
legend_string={'best NE in pure strategies (LC model)','worst NE in pure strategies (LC model)','optimal $n$-tuple of pure strategies (GC'''' model)','best symmetric NE in mixed strategies (F-LC model)','worst symmetric NE in mixed strategies (F-LC model)','optimal symmetric mixed strategy (F-GC'''' model)'};
legend(legend_string,'FontSize',11,'Location','Northeast','interpreter','latex');

ylim([0,90]);

xlabel('unit activation cost ($k$)','interpreter','latex','FontSize',11)
%ylabel('penalty 1 (convergence time)','interpreter','latex')
ylabel('(expected) $c_{convergence}$','interpreter','latex','FontSize',11)
...
ylabel('(expected) $c_{global}$','interpreter','latex','FontSize',11)
...
legend_string={'best NE in pure strategies for the LC model','worst NE in pure strategies (LC model)','optimal $n$-tuple of pure strategies (GC'' model)','best symmetric NE in mixed strategies (F-LC model)','worst symmetric NE in mixed strategies (F-LC model)','optimal symmetric mixed strategy (F-GC'' model)'};
legend(legend_string,'FontSize',11,'Location','Northeast','interpreter','latex');
...
ylabel('(expected) $c_{individual,max}$','interpreter','latex','FontSize',11)
